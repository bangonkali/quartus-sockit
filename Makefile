...
AR_REGEX := \
	...
	*.sip *.qip pll_vga top linux \
	*.sof *.rbf *.sopcinfo *.jdi output_files \
	hps_isw_handoff */*.svd *.dts *.dtb *board_info.xml \
...
ifneq ($(ACDS_VERSION),13.0.0)
IP_GENERATE_ARGS += --reportfile=svd
endif
...
$(DEVICE_TREE_SOURCE): %.dts: %.sopcinfo $(DTS.BOARDINFO)
	$(DTS.SOPC2DTS) --input $< --output $@ --board $(DTS.BOARDINFO)
